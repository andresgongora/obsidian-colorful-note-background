...
import ColorfulNoteBackgroundPlugin, {
    checkPath,
    hexToRgbA,
} from "../src/main";
...
jest.mock("obsidian", () => {
    ...
    const decorateElement = (element: HTMLElement): HTMLElement => {
        ...
        obsidianElement.addClass = (className: string) =>
            element.classList.add(className);
        obsidianElement.createEl = (
            tagName: string,
    ...
    class MockButtonComponent {
        ...
        onClick(callback: ClickHandler): this {
    ...
    class MockTextComponent {
        ...
        constructor(_containerEl: HTMLElement) {
            ...
            input.addClass = (className: string) =>
                this.inputEl.classList.add(className);
            input.toggleClass = (className: string, force: boolean) =>
                this.inputEl.classList.toggle(className, force);
        }
    ...
    class MockDropdownComponent {
        ...
        constructor(_containerEl: HTMLElement) {
            MockDropdownComponent.instances.push(this);
        }
    ...
    class MockColorComponent {
        ...
        constructor(_containerEl: HTMLElement) {
            MockColorComponent.instances.push(this);
        }
    ...
    return {
        ...
        PluginSettingTab: class {
            ...
            constructor(_app: any, _plugin: any) {
                this.containerEl = decorateElement(
...
describe("utility functions", () => {
    ...
    it("should not match filenames", () => {
        ...
        expect(checkPath("Index/300-Obsidian-index.md", "Obsidian")).toBe(
            false,
        );
    });
    ...
    it("should match paths with spaces", () => {
        expect(
            checkPath("Inbox/Test Note/test note with spaces.md", "Test Note"),
        ).toBe(true);
    });
...
describe("applyRule", () => {
    ...
    it("does not match malformed frontmatter rules without a value", () => {
        ...
        expect(
            plugin.applyRule(file, frontmatterRule("status"), contentView),
        ).toBe(false);
    ...
    it("does not apply folder rules that are absent from the file path", () => {
        const plugin = new ColorfulNoteBackgroundPlugin(
...
describe("plugin lifecycle and rule application", () => {
    ...
    it("loads settings, registers events, and adds its settings tab", async () => {
        ...
        expect((plugin as any).registeredEvents).toEqual([
            "active-leaf-change",
            "changed",
            "rename",
        ]);
        ...
        expect(plugin.applyRules).toHaveBeenNthCalledWith(2, {
            path: "changed.md",
        });
    ...
    it("applies only the first matching rule to the requested markdown file", () => {
        ...
        expect(first.contentView.classList.contains("cnb-highlighted")).toBe(
            true,
        );
        expect(
            first.contentView.style.getPropertyValue("--cnb-highlight-color"),
        ).toBe("rgba(17,34,51,0.2)");
        expect(second.contentView.classList.contains("cnb-highlighted")).toBe(
            true,
        );
    });
    ...
    it("clears markdown highlights when unloaded and uses default alpha when omitted", () => {
        ...
        highlighted.contentView.style.setProperty(
            "--cnb-highlight-color",
            "rgba(1,2,3,0.4)",
        );
        const app = {
        ...
        expect(
            highlighted.contentView.classList.contains("cnb-highlighted"),
        ).toBe(false);
        expect(
            highlighted.contentView.style.getPropertyValue(
                "--cnb-highlight-color",
            ),
        ).toBe("");
        expect(
            nonMarkdown.contentView.style.getPropertyValue(
                "--cnb-highlight-color",
            ),
        ).toBe("rgba(1,2,3,1)");
    });
...
describe("SettingsTab rule actions", () => {
    ...
    const buttons = (): any[] =>
        (jest.requireMock("obsidian").ButtonComponent as any).instances;
    const textInputs = (): any[] =>
        (jest.requireMock("obsidian").TextComponent as any).instances;
    const dropdowns = (): any[] =>
        (jest.requireMock("obsidian").DropdownComponent as any).instances;
    const colorPickers = (): any[] =>
        (jest.requireMock("obsidian").ColorComponent as any).instances;
    ...
    it("moves a middle rule up without changing the other order", () => {
        ...
        const moveUp = buttons().filter(
            (button) => button.icon === "chevron-up",
        )[1];

        moveUp.onClickCallback();

        expect(plugin.settings.colorRules.map((rule) => rule.id)).toEqual([
            "second",
            "first",
            "third",
        ]);
    ...
    it("moves a middle rule down without changing the other order", () => {
        const { plugin } = createTab();
        const moveDown = buttons().filter(
            (button) => button.icon === "chevron-down",
        )[1];
    ...
    it("disables move up for the first rule", () => {
        ...
        expect(
            buttons().filter((button) => button.icon === "chevron-up")[0]
                .disabled,
        ).toBe(true);
    });
    ...
    it("deletes the matching rule", () => {
        const { plugin } = createTab();
        const deleteButton = buttons().filter(
            (button) => button.icon === "x",
        )[1];
    ...
    it("adds a default folder rule", () => {
        ...
        const addButton = buttons().find(
            (button) => button.buttonText === "Add rule",
        );
    ...
    it("updates type and validates a frontmatter value", () => {
        ...
        expect(valueInput.inputEl.classList.contains("cnb-invalid-input")).toBe(
            false,
        );
    ...
    it("synchronizes color picker changes and accepts only alpha values from zero through one", () => {
        ...
        expect(plugin.settings.colorRules[0]).toMatchObject({
            color: "#abcdef",
            alpha: 0.5,
        });
